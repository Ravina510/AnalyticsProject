...
const Tlf_20 = (props) => {
  ...
  const getdata = (StartDate, EndDate) => {
    ...
    //     //   resultsObject[source] =
    //     //     (recordData[source] / 60 / DifferenceInHours) * 100;
    //     // }
    ...
    Axios.get(
      `http://${window.location.hostname}:3005/TLF_20/${StartDate}/${EndDate}`
      ).then(
      (result) => {
  ...
  useEffect(() => {
    ...
    const StartDate =
      [date1.getFullYear(), mnth1, day1].join("-") + " 00:00:00";

    const date2 = new Date(),
      mnth2 = ("0" + (date2.getMonth() + 1)).slice(-2),
      day2 = ("0" + date2.getDate()).slice(-2);
    const EndDate = [date2.getFullYear(), mnth2, day2].join("-") + " 23:59:59";
