...
const Tfms3_loading = (props) => {
  ...
  const optimumLoading = () => {
    ...
    const { columns, dataSource } = processFetchedData(filteredData);
    setColumns(columns);
  ...
  const getdata = (fromdate) => {
    ...
    Axios.get(
      `http:///${window.location.hostname}:3005/TFMS_03/${fromdate}`
    ).then((result) => {
  ...
  return isLoggedin ? (
    <div>
      ...
      <div style={{ padding: "20px", textAlign: "left" }}>
        ...
        <ul>
          <li>
            Average Sale Quantity(KL) = (Average Sale of last 7 days, excluding today) x (Stock
            for 2 days).
          </li>
          <li>
