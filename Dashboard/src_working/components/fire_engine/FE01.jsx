...
const FE01 = (props) => {
  ...
  const config = {
    ...
    // onEvent: (e) => console.log("Hello clicked", e.chart._events),
    onLegendClick: handleLegendClick,
    onDoubleClick: handleLegendClick,
  ...
  const getdata = (StartDate, EndDate) => {
    ...
    //     const percentage = (sourceOccurrences[source] / 12) * 100;
    //     sourcePercentageOccurrences[source] = percentage;
    //   }
    //   setPairOfPercAndOccur(sourcePercentageOccurrences);

    //   const getGridData = result.data.recordset.map(({ Counter, ...rest }) => rest);
    ...
    //   }
    // });
    AnalyticAxios.get(`/api/ColumnChart/FE_maintenance_more_than_month/${StartDate}/${EndDate}`)
    .then((result) => {
