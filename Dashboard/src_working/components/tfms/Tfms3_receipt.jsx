...
AnalyticAxios.get(`/api/ColumnChart/TFMS_03_ullage/${StartDate}/${EndDate}`)
.then((result) => {
  console.log(result.data.recordset);
  setData(result.data.recordset);
...
const productName = item.productname;
const msl = item.MSL;
const pumpableVolume = parseFloat(item.pumpablevolume);
const mbl = item.MBL;
const ullageVolume = parseFloat(item.Ullagevolume);
