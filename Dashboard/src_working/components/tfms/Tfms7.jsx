...
const Tfms7 = (props) => {
  ...
  const getdata = (StartDate, EndDate) => {
    ...
    AnalyticAxios.get(`/api/ColumnChart/TFMS7/${StartDate}/${EndDate}`)
    .then((result) => {
      const getGridData = result.data.recordset;
      // setData(result.data.recordset);
      console.log(result.data.recordset, "records");
      const sourceOccurrences = {};
      result.data.recordset.forEach((entry) => {
        const source = entry.Source;
        const currentStatus =
          entry["Current Status"] === null ? "null" : "sum";
  ...
  // ----Graph display
      const sourceOccurArray = Object.entries(sourceOccurrences);

      // Sort the array in descending order based on the count
      sourceOccurArray.sort((a, b) => b[1] - a[1]);

      // Convert the sorted array back to an object
      const sortedSourceOccurrences = Object.fromEntries(sourceOccurArray);

      console.log(sourceOccurArray, "sourceOccurArray");

      setSourcePercentages(sortedSourceOccurrences);

      // Create a map to store the counts for each source
      const sourceCounts = new Map();

      // Iterate over the data and update counts
      result.data.recordset.forEach((entry) => {
        const source = entry.Source;
        const currentStatus = entry["Current Status"];

        // If the source is not in the map, initialize count to 0
        if (!sourceCounts.has(source)) {
          sourceCounts.set(source, { Count: 0, Status: "Not Working" });
        }

        // If 'Current Status' is not null, increment the count and update status
        if (currentStatus !== null) {
          sourceCounts.set(source, {
            Count: sourceCounts.get(source).Count + 1,
            Status: "Working",
          });
        }
      });
      sourceCounts.set(null, { Count: 0, Status: '0' });

      // Convert the map to the desired output format
      const output = Array.from(sourceCounts, ([source, { Count, Status }]) => ({ Source: source, Count, Status }))
      .sort((a, b) => (a.Source === null ? -1 : b.Source === null ? 1 : a.Source.localeCompare(b.Source)));
