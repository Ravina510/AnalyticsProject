...
const Tlf_20 = (props) => {
  ...
  const getdata = (StartDate, EndDate) => {
    AnalyticAxios.get(`/api/GridChart/TLF_20/${StartDate}/${EndDate}`)
    .then((result) => {
      ...
      getGridData.forEach((row) => {           
        ...
        const { Source, 'Difference in Minutes': differenceInMinutes } = row;
        const numericValue = parseInt(differenceInMinutes, 10);
        if (!isNaN(numericValue)) {
          recordData[Source] = (recordData[Source] || 0) + numericValue;
        } else {
          recordData[Source] = recordData[Source] || 0;
        }
      });
      ...
      for (const source in recordData) {
        ...
        resultsObject[source] = (recordData[source] / 60 / DifferenceInHours) * 100;
      }
      ...
      setSourcePercentages(sortedData);
    ...
    // Axios.get(`http://${window.location.hostname}:3005/TLF_20/${StartDate}/${EndDate}`).then(
    //   (result) => {
    //     const getGridData = result.data.recordset;
    //     setData(result.data.recordset);
    ...
    //     // console.log(sortedSourceOccurrences);
