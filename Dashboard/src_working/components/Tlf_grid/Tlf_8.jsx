...
const Tlf_8 = () => {
  ...
  const getGriddata = (
    ...
    value2
      ...
      // Axios.get(
      //   `http://${window.location.hostname}:3005/TLF_8_Combined/${StartDate}/${EndDate}/${value1}/${value2}`
      // ).then((result) => {
      ...
      //   }
      // });
      AnalyticAxios.get(`/api/GridChart/TLF_8_Combined/${StartDate}/${EndDate}/${value1}/${value2}`)
      .then((result) => {
        const sumTTOccurence = result.data.recordsets[0].reduce((total, record) => {
          return total + record["Result Count"];
        }, 0);
  ...
  const loadData = (StartDate, EndDate, value1, value2) => {
    getGriddata("TLF_8_Combined", StartDate, EndDate, value1, value2);
  ...
  return isLoggedin ? (
    <div className="main">
      ...
      <div style={{ fontSize: "20px" }}>
        ...
        Cycle Time From:{"   "}
        <u>
          <strong>Card Issue Time{"   "}</strong>
        </u>
        To:{"   "}
        <u>
          <strong>Lic Entry Gate Time{"   "}</strong>
        </u>
        {"   "}
      </div>
