...
const Tlf_15 = () => {
  ...
  const getGriddata = (
    spName = "TLF_15_Combined",
    StartDate,
    EndDate,
    value
  ) => {
    if (spName === "TLF_15_Combined") {
      // Axios.get(
      //   `http://${window.location.hostname}:3005/TLF_15_Combined/${StartDate}/${EndDate}/${value}`
      // ).then((result) => {
      //   const sumTTOccurence = result.data.recordsets[1].reduce((total, record) => {
      ...
      //   }
      // });
      AnalyticAxios.get(`/api/ColumnChart/TLF_15_Combined/${StartDate}/${EndDate}/${value}`)
      .then((result) => {          
        const sumTTOccurence = result.data.recordsets[1].reduce((total, record) => {
          return total + record["Result Count"];
        }, 0);
  ...
  // const getBayGraphdata = (
  //   spName = "TLF_15_Combined_Bay",
  //   StartDate,
  //   EndDate,
  //   value
  // ) => {
  //   if (spName === "TLF_15_Combined_Bay") {
  //     Axios.get(
  //       `http://${window.location.hostname}:3005/TLF_15_Combined_Bay/${StartDate}/${EndDate}/${value}`
  //     ).then((result) => {
  //       // const sum = new Set(result.data.recordset.map((a) => a.TTSum));
  //       // const sumQty = [...sum].map((a) => a);
  //       // setDataCount(sumQty);
  //       // console.log("BayGraphData", sumQty);
  //       setData(list1);
  //     });
  //   } else {
  //   }
  ...
  const loadData = (StartDate, EndDate, value) => {
    getGriddata("TLF_15_Combined", StartDate, EndDate, value);
    // getBayGraphdata("TLF_15_Combined_Bay", StartDate, EndDate, value);
  ...
  return isLoggedin ? (
    <div className="main">
      ...
      <div className="column">
        ...
        >
          <Column style={{ width: "50rem" }} {...config} />
        </div>
      </div>
