...
const Tlf_2 = () => {
  ...
  const [columnsBay, setColumnsBay] = useState([]);
  const [dataGridBay, setDataGridBay] = useState([]);
  ...
  const getGriddata = (
    ...
    if (spName === "TLF_2_Combined") {
      AnalyticAxios.get(`/api/GridChart/TLF_2_Combined/${StartDate}/${EndDate}/${value1}/${value2}`)
      .then((result) => {
        const sumTTOccurence = result.data.recordsets[0].reduce(
          (total, record) => {
            return total + record["Result Count"];
          },
          0
        );
        setDataCount(sumTTOccurence === "" ? "0" : sumTTOccurence);
  ...
  const TotalCountOfCompartments = (
    ...
    if (spName === "TotalCountOfCompartments") {
      ...
      // ).then((result) => {
      //   const CountOfComp = result.data.recordset[0].CountOfCompartments;
      //   setCompDataCount(CountOfComp);
      //   console.log(CountOfComp, "TTCompartmentsCount");
      // });
  ...
  const loadData = (StartDate, EndDate, value1, value2) => {
    ...
    // getGriddataBay("TLF_2_Combined_Bay", StartDate, EndDate, value1, value2);
