...
const Tlf_6 = () => {
  ...
  const TotalCountOfTT = (
    ...
    if (spName === "TotalCountOfIndents") {
      // Axios.get(
      //   `http://${window.location.hostname}:3005/api/GridChart/TotalCountOfIndents/${StartDate}/${EndDate}`
      // ).then((result) => {
  ...
  const getGriddata = (
    ...
    if (spName === "TLF_6_Combined") {
      AnalyticAxios.get(`/api/GridChart/TLF_6_Combined/${StartDate}/${EndDate}`)
      .then((result) => {
        const sumTTOccurence = result.data.recordset.reduce((total, record) => {
          return total + record["Count of Indents/Bay"];
        }, 0);
      ...
      // Axios.get(
      //   `http://${window.location.hostname}:3005/api/GridChart/TLF_6_Combined/${StartDate}/${EndDate}`
      // ).then((result) => {
      //   const sumTTOccurence = result.data.recordset.reduce((total, record) => {
      //     return total + record["Count of Indents/Bay"];
      //   }, 0);
  ...
  const handleToday = (e) => {
    ...
    getGriddata("TLF_6_Combined", StartDate, EndDate);
